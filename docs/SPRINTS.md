# SPRINTS.md — Food Store · Plan de Implementación Feature-First

Cada sprint desarrolla features completas (backend + frontend juntos). Orden determinado por dependencias lógicas.
...
### Frontend — Layout Base (US-075, US-076, US-066, US-067)
...
- [ ] `PerfilForm` — Ver/editar nombre, apellido (US-061, US-062)
- [ ] `CambiarPasswordForm` — Validación y actualización (US-063)
...
- [ ] Validación pre-checkout: disponibilidad y precios (US-069, US-070)
- [ ] Confirmación visual de pedido creado (US-071)
...
- [ ] Polling de estado de pago cada 30s (US-072)
...
**Épicas:** EPIC 15, EPIC 16, EPIC 17, EPIC 18
...
- [ ] `ConfiguracionPanel` — Key-value de configuración del sistema (US-060)
...
```
Sprint 0 (base) → Sprint 1 (auth) → Sprint 2 (catálogo base)
→ Sprint 3 (productos) → Sprint 4 (carrito+direcciones)
→ Sprint 5 (checkout) → Sprint 6 (pagos+FSM)
→ Sprint 7 (visualización) → Sprint 8 (admin completo)
```

Cada sprint depende del anterior. No comenzar un sprint sin que el anterior esté completo y testeado.
